...
fn call_agent_sidecar(
    ...
    let mut child = Command::new(&tsx_bin)
        ...
        .stderr(Stdio::piped())
        .spawn()
        .map_err(|e| format!("Failed to start agent sidecar at {}: {}", sidecar_js.display(), e))?;
    ...
    if !output.status.success() {
        ...
        return Err(format!("Agent sidecar exited with error: {}", stderr));
    }
    ...
    if let Some(error) = response.error {
        return Err(format!("Agent error {}: {}", error.code, error.message));
    }

    response
        .result
        .ok_or_else(|| "No result in agent response".to_string())
}
...
pub fn export_pptx(
    ...
    // Write deck JSON to a temp file
    let output_dir = app
        .path()
        .app_data_dir()
...
pub fn export_html(
    ...
    let request = JsonRpcRequest {
        id: "export_html_001".to_string(),
        method: "render.html".to_string(),
        params: serde_json::json!({
            "deckPath": deck_path.to_string_lossy(),
            "outputPath": output_path.to_string_lossy(),
        }),
