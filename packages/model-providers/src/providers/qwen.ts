...
export function createQwenProvider(
  ...
  const baseUrl = options.baseUrl ?? QWEN_DEFAULT_BASE_URL;
  const apiKey = options.apiKey ?? "";
  const model = options.model ?? QWEN_DEFAULT_MODEL;
  ...
  return {
    ...
    async complete(req: TextCompletionRequest): Promise<TextCompletionResult> {
      ...
      try {
        ...
        throw new Error(
          `Failed to connect to Qwen API at ${baseUrl}: ${err instanceof Error ? err.message : String(err)}`
        );
      }
